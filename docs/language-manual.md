...
!!! note "Qualified types"
    ...
    Numeric literals are translated by the compiler to a function call, for example:

    ```
    from_int32(42)
    ```

    The type of `from_int32` is `int32 -> n with (Numeric<n>)`. When you define an expression involving a qualified type, e.g.:
    ...
    the compiler needs to add a hidden parameter to the enclosing function which passes the instance of the trait in question. 
    In the case of numeric types and `from_int32`, the trait is `Numeric`.
    This .. Numeric<int32> or Numeric<nat>

    The final code generated by the compiler will then look something like this:
    ...
    This is how literals like `42` _ polymorphism.
