...
!!! note "Qualified types"
    ...
    the compiler adds a hidden parameter to the enclosing function to supply the instance of the trait in question. 
    ...
    The function `do_stuff` is specialized to a specific type by calling it with an instance like `Numeric<int32>` or `Numeric<nat>`.
